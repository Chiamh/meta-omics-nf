...
awk -v OFS="\t" '{print $2,$12}' "${dmnd_results}" | sort -k 1,1 | uniq > "${sample_id}".sizes
...
awk -v OFS="\t" '{print $2,$10-1,$11}' "${dmnd_results}" | sort -k 1,1 > "${sample_id}".bed
...
| join -a 1 -1 1 -2 2 -t $'\t' - <(awk -v OFS="\t" '{print $2}' "${dmnd_results}" \
| sort | uniq -c | sed 's/^ *//; s/ /\t/') | join -a 1 -1 1 -2 1 -t $'\t' - <(cut -f 2,15 "${dmnd_results}" | sort -t $'\t' -k 1,1 | uniq) \
