...
samtools view -F 4 "${bam}" | cut -f 1,3 | sort -k1,1 > "${sample_id}"_read_to_pangene.tsv
...
join -a 2 -1 1 -2 1 -e '-' -o auto -t $'\t' "${sample_id}"_k2_read_sorted "${sample_id}"_read_to_pangene.tsv | sort -t $'\t' -k3,3 \
| join -a 1 -1 3 -2 1 -t $'\t' -e '-' -o auto - <(awk -F "\t" -v OFS="\t" '(NR>1){print $1,$3}' "${pangenome_annots}") > "${sample_id}"_pangene_taxonomy.tsv

join -a 2 -1 1 -2 1 -e '-' -o auto -t $'\t' "${sample_id}"_k2_read_sorted <(cut -f1,2 "${sample_id}"_uniref90_aligned.out | sort -k1,1) \
